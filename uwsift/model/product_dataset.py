...
class ProductDataset:
    ...
    def get_rgb_multichannel_product_dataset(
        ...
        if not any(input_datasets_uuids):
            LOG.debug(f"Could not create a multichannel ProductDataset," f" when no input ProductDatasets exist.")
            return None
    ...
    def _update_info_with_input_infos(dataset_info, input_datasets_infos) -> dict:
        ...
        if len(infos_to_compare) == 0:
            LOG.debug(f"Could not create a multichannel ProductDataset," f" without any given datasets infos")
            return None
    ...
    def update_multichannel_dataset_info(self, input_datasets_infos):
        if not self.input_datasets_uuids:
            LOG.debug(f"Can't update multichannel dataset info if the dataset" f" is not a multichannel dataset.")
            return None

        if not any(input_datasets_infos):
            LOG.debug(f"Can't update multichannel dataset info if no" f" valid input datasets infos available.")
            return None
    ...
    def get_algebraic_dataset(
        ...
        if not any(input_datasets_uuids):
            LOG.debug(f"Could not create a algebraic ProductDataset," f" when no input ProductDatasets exist.")
            return None
