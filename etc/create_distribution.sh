...
SCR_TEMPLATE="./sift-dist-template"
...
cp -R "$SCR_TEMPLATE" "$DIST_OUTPUT/sift-dist"
#copy the default settings
cp -R "../resources" "$DIST_OUTPUT/sift-dist"

#copy the built pyinstaller version as lib in the dist
cp -R "$HOME_PYINSTALLER_BUILD/dist/sift" "$DIST_OUTPUT/sift-dist/lib"

#move out the libraries creating some dependencies conflicts (keep them for the moment)
mv "$DIST_OUTPUT/sift-dist/lib/libudev.so.1" "$DIST_OUTPUT/sift-dist/etc/extra-libs"
mv "$DIST_OUTPUT/sift-dist/lib/libxcb-shm.so.0" "$DIST_OUTPUT/sift-dist/etc/extra-libs"

#Adapt Qt5 if necessary
PYQT5LIB="$DIST_OUTPUT/sift-dist/lib/PyQt5/Qt"
#copy icu file
...
mkdir -p "$DIST_OUTPUT/sift-dist/lib/gribapi/"
cp "$CONDA_PREFIX/lib/python3.10/site-packages/gribapi/grib_api.h" "$DIST_OUTPUT/sift-dist/lib/gribapi/"
cp "$CONDA_PREFIX/lib/python3.10/site-packages/gribapi/eccodes.h" "$DIST_OUTPUT/sift-dist/lib/gribapi/"
