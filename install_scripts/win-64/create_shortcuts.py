...
update_shortcut.Arguments = (
    "/c activate {env_name} && conda update -y -c {channel} {pkg_name} && pause && deactivate".format(
        env_name=env_name, pkg_name=pkg_name, channel=channel
    )
)
...
run_shortcut.Arguments = "/c activate {env_name} && python -m {pkg_name} -w {workspace} && pause && deactivate".format(
    env_name=env_name, pkg_name=pkg_name, workspace=workspace
)
run_shortcut.save()
